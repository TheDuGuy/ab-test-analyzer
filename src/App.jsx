...
function App() {
  ...
  const exportToExcel = () => {
    ...
    const data = [
      ...
      [],
      ['Test Configuration'],
      ['Group', config.metric1, config.metric2, config.rate, '95% CI Lower', '95% CI Upper'],
      ['Control', controlVisitors, controlConversions, `${controlRate.toFixed(2)}%`, `${controlCI.lower.toFixed(2)}%`, `${controlCI.upper.toFixed(2)}%`],
      ['Variant', variantVisitors, variantConversions, `${variantRate.toFixed(2)}%`, `${variantCI.lower.toFixed(2)}%`, `${variantCI.upper.toFixed(2)}%`],
      [],
      ['Results'],
      ['Metric', 'Value'],
      ['Winner', result.winner === 'inconclusive' ? 'Inconclusive' : result.winner],
      ['Relative Uplift', `${relativeUplift >= 0 ? '+' : ''}${relativeUplift.toFixed(2)}%`],
      ['Absolute Change', `${Math.abs(variantRate - controlRate).toFixed(2)}pp`],
      [],
      ['Statistical Details'],
      ['Z-Score', stats.zScore.toFixed(4)],
      ['P-Value', stats.pValue.toFixed(4)],
      ['Statistically Significant (95%)', stats.isSignificant ? 'Yes' : 'No'],
      ['Confidence Level', `${stats.confidenceLevel.toFixed(1)}%`],
      ['Total Sample Size', (controlVisitors + variantVisitors).toLocaleString()],
    ]

    const ws = XLSX.utils.aoa_to_sheet(data)
    const wb = XLSX.utils.book_new()
    XLSX.utils.book_append_sheet(wb, ws, 'AB Test Results')
    XLSX.writeFile(wb, `ab-test-results-${channel}-${timestamp}.xlsx`)
  }
  ...
  const exportToPDF = () => {
    ...
    doc.text('A/B Test Results Analysis', 105, 20, { align: 'center' })

    // Date and Channel
    doc.setFontSize(10)
    doc.setTextColor(100, 100, 100)
    doc.text(`Generated on: ${timestamp}`, 20, 35)
    doc.text(`Channel: ${channel.toUpperCase()}`, 20, 42)

    // Test Configuration
    doc.setFontSize(14)
    doc.setTextColor(0, 0, 0)
    doc.text('Test Configuration', 20, 55)

    doc.setFontSize(10)
    let y = 65

    // Control Group
    doc.text('Control Group:', 20, y)
    doc.text(`${config.metric1}: ${controlVisitors.toLocaleString()}`, 30, y + 7)
    doc.text(`${config.metric2}: ${controlConversions.toLocaleString()}`, 30, y + 14)
    doc.text(`${config.rate}: ${controlRate.toFixed(2)}%`, 30, y + 21)
    doc.text(`95% CI: ${controlCI.lower.toFixed(2)}% - ${controlCI.upper.toFixed(2)}%`, 30, y + 28)

    y += 40

    // Variant Group
    doc.text('Variant Group:', 20, y)
    doc.text(`${config.metric1}: ${variantVisitors.toLocaleString()}`, 30, y + 7)
    doc.text(`${config.metric2}: ${variantConversions.toLocaleString()}`, 30, y + 14)
    doc.text(`${config.rate}: ${variantRate.toFixed(2)}%`, 30, y + 21)
    doc.text(`95% CI: ${variantCI.lower.toFixed(2)}% - ${variantCI.upper.toFixed(2)}%`, 30, y + 28)
    ...
    doc.setFontSize(10)
    doc.setTextColor(result.winner === 'inconclusive' ? 100 : (variantRate > controlRate ? 34 : 59), 100, result.winner === 'inconclusive' ? 100 : (variantRate > controlRate ? 139 : 130))
    doc.text(`Winner: ${result.winner === 'inconclusive' ? 'Inconclusive' : result.winner}`, 20, y)

    doc.setTextColor(0, 0, 0)
    y += 7
    doc.text(`Relative Uplift: ${relativeUplift >= 0 ? '+' : ''}${relativeUplift.toFixed(2)}%`, 20, y)
    y += 7
    doc.text(`Absolute Change: ${Math.abs(variantRate - controlRate).toFixed(2)}pp`, 20, y)

    y += 15

    // Statistical Details
    doc.setFontSize(14)
    doc.text('Statistical Details', 20, y)
    y += 10

    doc.setFontSize(10)
    doc.text(`Z-Score: ${stats.zScore.toFixed(4)}`, 20, y)
    y += 7
    doc.text(`P-Value: ${stats.pValue.toFixed(4)}`, 20, y)
    y += 7
    doc.text(`Statistically Significant (95%): ${stats.isSignificant ? 'Yes' : 'No'}`, 20, y)
    y += 7
    doc.text(`Confidence Level: ${stats.confidenceLevel.toFixed(1)}%`, 20, y)
    y += 7
    doc.text(`Total Sample Size: ${(controlVisitors + variantVisitors).toLocaleString()}`, 20, y)
    ...
    doc.setFontSize(9)
    const recommendation = !stats.isSignificant
      ? "The test results are not statistically significant at the 95% confidence level. Consider running the test longer or with more traffic to reach significance."
      : stats.isSignificant && variantRate > controlRate
        ? "The variant shows a statistically significant improvement over the control. Consider implementing this variation."
        : "The control performs significantly better than the variant. Keep the current version or test a different variation."
    ...
    doc.setFontSize(8)
    doc.setTextColor(100, 100, 100)
    doc.text('Generated by A/B Test Results Analyzer', 105, 285, { align: 'center' })
    doc.text('Built by Edou Mota', 105, 290, { align: 'center' })

    doc.save(`ab-test-results-${channel}-${timestamp.replace(/\//g, '-')}.pdf`)
  }
